...
Maintainer guide for publishing a new version of `@piwikpro/next-piwik-pro` to npm.
...
2. Bump the version. This does **not** create a commit or tag — `.npmrc` sets
   ...
   `version` npm lifecycle hook still runs, so it also regenerates the changelog
   (adds the new version heading + commits since the last release) and bakes the
   version into the build (`src/version.ts`):
...
- When you create the GitHub Release, type the tag **unprefixed** (`3.5.1`, not
  `v3.5.1`). This matches the existing tag history and the changelog compare
